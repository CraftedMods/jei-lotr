...
public class JEIPlugin implements IModPlugin
{
    ...
    public void initDevices ()
    {
        if (devices == null)
        {
            ...
            scanFields (LOTRRecipes.class.getName (), LOTRContainers.class.getName (), "lotr");

            if (ModList.get ().isLoaded ("lotrfa"))
            {
                scanFields ("eoa.lotrfa.common.recipe.LOTRFARecipes", "eoa.lotrfa.common.init.LOTRFAContainers",
                    "lotrfa");
            }
            ...
            LOTRAlloyForge orcForgeDevice = new LOTRAlloyForge (new ResourceLocation ("lotr", "orc_forge"),
                new ItemStack (LOTRBlocks.ORC_FORGE.get ()), Arrays.asList (IRecipeType.SMELTING, LOTRRecipes.ORC_FORGE,
                    LOTRRecipes.ALLOY_FORGE, LOTRRecipes.ORC_FORGE_ALLOY),
    ...
    private void scanFields (String recipeClass, String containerclass, String modid)
    {
        ...
        {
            JEILotr.LOGGER.error ("Couldn't instantiate the faction crafting tables for " + modid, e);
        }
    }
    ...
    public void registerCategories (IRecipeCategoryRegistration registration)
    {
        ...
        {
            registration.addRecipeCategories (device.createCategoryInstance (
                registration.getJeiHelpers ().getGuiHelper (), registration.getJeiHelpers ().getModIdHelper ()));
        });
    }
            ...
            try
            {
                ...
                for (IRecipeType<?> type : device.recipeTypes)
                {
                    recipes.addAll ( ((Map<ResourceLocation, IRecipe<?>>) getRecipesMethod
                        .invoke (minecraft.world.getRecipeManager (), type)).values ().stream ()
